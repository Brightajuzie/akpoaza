...
async function verifyOpayTransaction(
  reference: string,
  merchantId: string,
  publicKey: string,
  secretKey: string,
): Promise<{ verified: boolean; amount?: number; currency?: string }> {
  ...
  const verified = response.data?.code === '00000' && data?.status === 'SUCCESS';
  return {
    verified,
    amount: data?.amount?.total ? Number(data.amount.total) / 100 : undefined,
...
function verifyOpayWebhookSignature(payload: any, secretKey: string): boolean {
  const received = payload?.sha512 || payload?.data?.sha512;
  if (!received || typeof received !== 'string') return false;

  const d = payload?.data || payload;
  const signContent =
    `{Amount:"${d.amount}",Currency:"${d.currency}",Reference:"${d.reference}",` +
    `Refunded:${d.refunded ? 't' : 'f'},Status:"${d.status}",Timestamp:"${d.timestamp}",` +
    `Token:"${d.token}",TransactionID:"${d.transactionId ?? d.orderNo}"}`;

  const expected = crypto.createHmac('sha3-512', secretKey).update(signContent).digest('hex');
  try {
...
async function processPaymentVerification({
  ...
  if (checkoutType === 'order') {
    ...
    if (!funding) throw new Error('Wallet funding request not found');
    ...
    const updatedFunding = await prisma.walletFunding.update({
      where: { id },
      data: {
        status: 'PAID',
        paymentProvider: sanitizePaymentProvider(provider),
    ...
    const wallet = await getOrCreateWallet(funding.userId);
...
function renderFailureHtml(provider: string, reference: string, frontendUrl: string, message?: string) {
...
async function renderPaymentSuccessPage({
  ...
  if (checkoutType === 'order' || checkoutType === 'booking' || checkoutType === 'parcel') {
    try {
      const receipt = await generateReceiptData(checkoutType, id);
      if (receipt) {
        const returnUrl = `${frontendUrl}/?payment_status=success&ref=${encodeURIComponent(reference)}`;
        return renderReceiptHtml(receipt, { returnUrl });
      }
    } catch (err) {
...
router.post('/checkout', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (checkoutType === 'order') {
      ...
      if (!funding) return res.status(404).json({ error: 'Wallet funding request not found' });
      if (funding.status === 'PAID') return res.status(400).json({ error: 'This funding request was already paid.' });
...
router.get('/paystack/verify/:reference', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (isDummy) {
      ...
      const funding = await prisma.walletFunding.findUnique({ where: { id } });
      if (funding) {
        const result = await processPaymentVerification({ provider: 'PAYSTACK', reference, checkoutType: 'wallet_funding', id, chargedAmount: funding.amount });
        return res.json({ status: 'success', message: 'Wallet funding verified via Paystack sandbox.', ...result });
      }
...
router.get('/paystack/callback', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!isDummy) {
      try {
        ...
        if (verifyRes.data?.data?.status !== 'success') {
          return res.send(renderFailureHtml('Paystack', reference, frontendUrl, `Payment status: ${verifyRes.data?.data?.status || 'unknown'}.`));
        }
      ...
      if (order) {
        ...
        if (booking) {
          ...
          if (parcel) {
            ...
            const funding = await prisma.walletFunding.findUnique({ where: { id } });
            if (funding) {
              await processPaymentVerification({ provider: 'PAYSTACK', reference, checkoutType: 'wallet_funding', id, chargedAmount: chargedAmountOverride || funding.amount });
    ...
    res.send(await renderPaymentSuccessPage({ checkoutType: matchedType, id, provider: 'Paystack', reference, frontendUrl }));
  } catch (error) {
...
router.get('/flutterwave/verify/:transactionId', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (isDummy) {
      ...
      const funding = await prisma.walletFunding.findUnique({ where: { id } });
      if (funding) {
        const result = await processPaymentVerification({ provider: 'FLUTTERWAVE', reference: transactionId, checkoutType: 'wallet_funding', id, chargedAmount: funding.amount });
        return res.json({ status: 'success', message: 'Wallet funding verified via Flutterwave sandbox.', ...result });
      }
...
router.get('/flutterwave/callback', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
      ...
      if (order) {
        ...
        if (booking) {
          ...
          if (parcel) {
            ...
            const funding = await prisma.walletFunding.findUnique({ where: { id } });
            if (funding) {
              await processPaymentVerification({ provider: 'FLUTTERWAVE', reference: txRef, checkoutType: 'wallet_funding', id, chargedAmount: chargedAmountOverride || funding.amount });
              matchedType = 'wallet_funding';
            }
          }
        }
    ...
    res.send(await renderPaymentSuccessPage({ checkoutType: matchedType, id, provider: 'Flutterwave', reference: txRef, frontendUrl }));
  } catch (error) {
  ...
  try {
    ...
    if (!isDummy) {
      ...
      try {
        ...
        id = pi.metadata?.id || null as any;
        checkoutType = pi.metadata?.checkoutType || null;
    ...
    if (order) {
      ...
      if (booking) {
        ...
        if (parcel) {
          ...
          const funding = await prisma.walletFunding.findUnique({ where: { id } });
          if (funding) {
            await processPaymentVerification({ provider: 'STRIPE', reference, checkoutType: 'wallet_funding', id, chargedAmount: chargedAmountOverride || funding.amount });
            matchedType = 'wallet_funding';
          }
        }
      }
    ...
    res.send(await renderPaymentSuccessPage({ checkoutType: matchedType, id, provider: 'Stripe', reference, frontendUrl }));
  } catch (error) {
...
router.get('/opay/verify/:reference', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const opaySettings = await prisma.appSetting.findMany({
      where: { key: { in: ['opay_merchant_id', 'opay_public_key', 'opay_secret_key'] } },
    });
    const os = opaySettings.reduce((acc, s) => ({ ...acc, [s.key]: s.value }), {} as Record<string, string>);
    ...
    if (!isDummy) {
      try {
        const result = await verifyOpayTransaction(reference, merchantId, publicKey, secretKey);
        if (!result.verified) {
...
router.post('/opay/webhook', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const opaySettings = await prisma.appSetting.findMany({ where: { key: 'opay_secret_key' } });
    const secretKey = opaySettings[0]?.value || process.env.OPAY_SECRET_KEY || '';
    ...
    if (ref) {
      ...
      if (id) {
        ...
        if (funding && funding.status !== 'PAID') {
          await processPaymentVerification({ provider: 'OPAY', reference: ref, checkoutType: 'wallet_funding', id, chargedAmount: funding.amount });
        }
      }
    }
...
router.post('/webhook', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const event = activeStripe.webhooks.constructEvent(req.body, sig as string, endpointSecret);
...
router.post('/admin/force-release-escrow/:escrowId', authenticateToken, async (req: AuthRequest, res: Response, next: NextFunction) => {
